...
public class EntityAITofunianInteract extends EntityAIWatchClosest2
{
    /** The delay before the villager throws an itemstack (in ticks) */
    private int interactionDelay;
    private final EntityTofunian villager;

    public EntityAITofunianInteract(EntityTofunian villagerIn)
    {
        super(villagerIn, EntityTofunian.class, 3.0F, 0.02F);
    ...
    public void startExecuting()
    {
        ...
        if (this.villager.canAbondonItems() && this.closestEntity instanceof EntityTofunian && ((EntityTofunian)this.closestEntity).wantsMoreFood())
        {
            ...
            if (this.interactionDelay == 0)
            {
                ...
                for (int i = 0; i < inventorybasic.getSizeInventory(); ++i)
                {
                    ...
                    if (!itemstack.isEmpty())
                    {
                        ...
                        }else if (item == ItemLoader.rice && itemstack.getCount() > 0)
                        {
                            int j = itemstack.getCount();
                            itemstack.shrink(j);
                            itemstack1 = new ItemStack(ItemLoader.foodset, j, 2);
                        }
